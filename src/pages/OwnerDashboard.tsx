  ...
  return (
    <div className="max-w-4xl space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="heading text-2xl lg:text-3xl">Staff Management</h1>
          <p className="text-xs text-navy-500 dark:text-cream-200/60 mt-0.5">Track real employees and toggle Present / Absent attendance</p>
        </div>
      ...
      {loading ? <div className="grid place-items-center py-20"><Loader2 className="w-8 h-8 animate-spin text-maroon-700" /></div> : staff.length === 0 ? (
        <div className="card p-10 text-center">
          <Users className="w-10 h-10 text-navy-300 dark:text-cream-200/20 mx-auto mb-3" />
          <p className="font-sans font-bold text-navy-900 dark:text-cream-50">No staff members registered</p>
          <p className="text-xs text-navy-500 dark:text-cream-200/60 mt-1">Click 'Add Staff Member' above to create employee records.</p>
        </div>
      ) : (
        <div className="card overflow-hidden">
          <table className="w-full text-sm">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 glass-overlay grid place-items-center p-4">
          <div className="card p-6 max-w-md w-full space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="font-sans font-bold text-lg text-navy-900 dark:text-cream-50">Add Staff Member</h2>
              <button onClick={() => setIsModalOpen(false)} className="text-navy-400"><X className="w-5 h-5" /></button>
            </div>
            <form onSubmit={handleAddStaff} className="space-y-3 text-xs">
              <div>
                <label className="block font-semibold mb-1">Full Name</label>
                <input required value={newStaff.name} onChange={e => setNewStaff({ ...newStaff, name: e.target.value })} placeholder="e.g. Rahul Verma" className="w-full rounded-xl px-3 py-2 bg-cream-50 dark:bg-navy-800 border" />
              </div>
              <div>
                <label className="block font-semibold mb-1">Phone Number</label>
                <input value={newStaff.phone} onChange={e => setNewStaff({ ...newStaff, phone: e.target.value })} placeholder="+91 98765 43210" className="w-full rounded-xl px-3 py-2 bg-cream-50 dark:bg-navy-800 border" />
              </div>
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block font-semibold mb-1">Role</label>
                  <input value={newStaff.role} onChange={e => setNewStaff({ ...newStaff, role: e.target.value })} placeholder="Tea Maker / Server" className="w-full rounded-xl px-3 py-2 bg-cream-50 dark:bg-navy-800 border" />
                </div>
                <div>
                  <label className="block font-semibold mb-1">Shift</label>
                  <input value={newStaff.shift} onChange={e => setNewStaff({ ...newStaff, shift: e.target.value })} placeholder="Morning / Evening" className="w-full rounded-xl px-3 py-2 bg-cream-50 dark:bg-navy-800 border" />
                </div>
              </div>
              <div className="flex gap-2 pt-2">
                <button type="submit" className="btn-primary flex-1 py-2 text-xs">Add Staff</button>
                <button type="button" onClick={() => setIsModalOpen(false)} className="btn-outline flex-1 py-2 text-xs">Cancel</button>
              </div>
            </form>
          </div>
