...
import { Mail, Lock, ArrowRight, ArrowLeft, Eye, EyeOff, Loader2, User, Phone, KeyRound, CheckCircle2, RefreshCw } from 'lucide-react';
...
export function CustomerLoginPage() {
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const { error: err } = await verifyOtp(target, otpToken.trim(), otpChannel, 'customer');
      if (err) {
        setError(err);
      } else {
        localStorage.setItem('role', 'customer');
      ...
      <div className="w-full max-w-md">
        ...
        <div className="card p-7">
          ...
          <div className="text-center mb-6">
            <div className="mx-auto mb-3 grid place-items-center w-14 h-14 rounded-2xl bg-maroon-50 dark:bg-navy-800 text-maroon-700 dark:text-gold-300">
              {loginMethod === 'otp' ? <KeyRound className="w-7 h-7" /> : <User className="w-7 h-7" />}
            </div>
            ...
            <p className="prose-body text-sm mt-1">
              {isSignUp ? 'Join Deccan Chai and start ordering' : 'Sign in to track orders and earn rewards'}
            </p>
          </div>
              ...
              {isSignUp && (
                <>
                  <Field icon={User} label="Full Name">
                    <input type="text" required value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="Your Name" className="w-full bg-transparent focus:outline-none text-sm text-navy-900 dark:text-cream-50" />
                  </Field>
                  <Field icon={Phone} label="Phone">
                    <input type="tel" required value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+91 9876543210" className="w-full bg-transparent focus:outline-none text-sm text-navy-900 dark:text-cream-50" />
                  </Field>
                </>
              )}
              <Field icon={Mail} label="Email">
                <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@example.com" className="w-full bg-transparent focus:outline-none text-sm text-navy-900 dark:text-cream-50" />
              </Field>
          ...
          <p className="text-center mt-5 text-sm text-navy-600 dark:text-cream-200/70">
            ...
            <button type="button" onClick={() => { setIsSignUp(!isSignUp); setError(null); }} className="font-semibold text-maroon-700 dark:text-gold-300 hover:underline">
